...
Implemented the `pytest` absence probe in `validate.sh` before running `test/test_python_layer.py`.
When `pytest` is not importable, `validate.sh` logs `SKIPPED: python:test_python_layer.py (pytest not importable — install it to cover utils/py/)`, adds the suite to `SKIPPED_SUITES`, avoids incrementing `TOTAL`, and does not record a failure.
Created test suite `test/gh251-validate-pytest-skip.sh` and baseline `test/baselines/GH-251-negative-control.md`, and registered `gh251-validate-pytest-skip.sh` in `validate.sh`.

### Round 6 · Reviewer · codex

Reviewed the changes in `validate.sh`, `test/gh251-validate-pytest-skip.sh`, and `test/baselines/GH-251-negative-control.md`.
All acceptance criteria are satisfied:
1. `python3 -c "import pytest"` probe gates the pytest execution and skips cleanly when absent.
2. Genuine pytest failures still land in `FAILED`.
3. Test suite `gh251-validate-pytest-skip.sh` passes 5/0 and `gh306-registry-bidirectional.sh` passes 10/0.
