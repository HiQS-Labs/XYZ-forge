...
**Evidence:** every claim below cites the log it came from in `audit/logs/` — the raw stdout/stderr of a
command that ran, opening with the command line and closing with its exit code. Index: `audit/LOG-INDEX.md`.
**Reproduction:** the probe script that produced these logs is proposed in #51; no agent
credentials, no network and no token spend are needed to run it.
...
**Evidence:** `audit/logs/d1-1-what-the-default-is.log` (the lane switch and the guard passing),
`d1-2-real-tick-under-python.log` (WinError 193 against the **real** `bin/tick`, not a fixture wrapper),
`d1-3-default-turn.log` (the full traceback), `d1-4-after.log`.
...
**Evidence:** the worktree `--check` output quoted above; `audit/logs/validate-summary.log` for the
gh391 instance.
...
**Evidence:** `audit/logs/c8-bash-2-agent.log` (the forking agent), `c8-bash-3-verdict.log`
(exit 7 plus the post-wait marker check), `c8-bash-4-after.log` (the marker sitting in ROOT).
...
|---|---|---|---|---|
| **C0** | well-behaved agent (control) | exit 0, file-scoped commit, no push | **exit 0**, committed, no push | `c0-bash-*.log` |
| **C1** | `git commit` mid-turn, in-ROOT | prior HEAD → `refs/relay-orphan/`, `reset --hard`, exit 6 (`relay-turn-lib.sh:1080-1084`) | **exit 6**; HEAD back at seed; `sneaky.md` gone; `refs/relay-orphan/4fd12bb` present | `c1-bash-*.log` |
| **C2** | edit a tracked file outside the allowlist | reverted, exit 6 (`relay-turn-lib.sh:1117`) | **exit 6**; `untouchable.md` restored to baseline | `c2-bash-*.log` |
| **C3** | sleep 120s against a 5s ceiling | killed, exit 7 (`relay-turn-lib.sh:464-488`) | **exit 7**, killed on schedule | `c3-bash-*.log` |
| **C4** | exit 0 having done nothing | shim must own the token itself or exit 5 (`codex-turn.sh:124-129`) | **exit 0** — shim claimed the token, committed nothing, warned that STATUS was non-terminal with no `RELAY_PEER` | `c4-bash-*.log` |
| **C5** | off-lane edit under `RELAY_WORKTREE_ISOLATION=1` | worktree destroyed, nothing copied back, exit 6 | **exit 6**, ROOT untouched | `c5-bash-*.log` |
| **C6** | `git commit` under worktree isolation | **NOT** a reset — peer-preserve (`relay-turn-lib.sh:1064-1071`) | **preserved**, turn continued — correct | `c6-bash-*.log` |
| **C7** | off-lane edit **and** timeout together | 6 outranks 7 (`relay-turn-lib.sh:56-60`) | **exit 6**, off-lane reverted | `c7-bash-*.log` |
...
./validate.sh --sequential 2>&1 | tee /c/tmp/validate-run.log
```
...
**Evidence:** `audit/logs/gate-recursion.log` (the process tree showing the nested `validate.sh`),
`audit/logs/validate-summary.log` (the run's final state at kill time, with the failure-signature
counts below). The full 200KB `validate.sh` transcript those two were distilled from is not committed —
the distilled logs carry the claim, and the raw dump is regenerable by the command above.
...
| Signature in the `validate.sh` transcript | Count | Root cause |
|---|---|---|
