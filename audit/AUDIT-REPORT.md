# XYZ-forge — Technical Audit & QA Report

**Repo:** `github.com/HiQS-Suite/XYZ-forge` (cloned to `C:\Users\<user>\XYZ-forge`)
...
Every claim below is backed by a captured command transcript, so the maintainer can check any of them
...
| **Results table** | `audit/CONTAINMENT-RESULTS.md` | Machine-generated verdict table, emitted by the probe run. |
| **Command transcripts** | `audit/logs/*.log` + `audit/LOG-INDEX.md` | 48 captured logs. Each probe is a four-log sequence — before → the hostile agent → the harness verdict → the post-state proving the undo. Every log is the raw stdout/stderr of a command that ran, ending in its exit code. |
...
> **The repro tooling is not in this PR.** The probe scripts that produced these logs
> (`repro.sh`, `repro-containment.sh`) are proposed separately in #51, so this contribution stays an
> evidence record and the executable tooling can be reviewed on its own merits. See `README.md`.
...
**Diagrams:** `audit/CONTAINMENT-FLOW.mmd` traces every decision point and exit code from source,
annotated with the probe that verified it; `audit/TURN-SEQUENCE.mmd` walks one hostile turn from
dispatch to `git reset --hard`, with the SHAs taken from the actual probe log. Both are Mermaid
sources — GitHub renders them inline, so they need no committed image.
...
Observed live while driving `relay-drive.sh` against a throwaway repo. Summary:
...
**Top recommendation:** a lightweight **live dashboard** — a proposal, not something in the repo — that
...
## 7. Verification log (what was actually run)
