...
#   bash audit/repro-containment.sh --out DIR       # write logs/screens/table to DIR
#
# THIS SCRIPT WRITES NOTHING INTO THE REPO. Logs, screenshots and the results
# table go to --out (default: a directory inside the scratch dir), so a
# verification run can never overwrite a committed audit record.
#
# ENV OVERRIDES:
#   XYZ_UNDER_TEST  harness root to probe (default: this script's repo)
#   CHROME_BIN      headless browser for screenshots (auto-detected)
#   PROBE_OUT       same as --out
#   PROBE_TMP       scratch dir for fixtures. Default: a fresh `mktemp -d`.
#                   If you set it, it must be an ABSOLUTE path that is either
#                   missing (it will be created) or an EMPTY directory. A
#                   non-empty directory is refused, because this script deletes
#                   its scratch dir on the way out.
#
...
KEEP=0; SHOTS=1; ONLY_LANE=""; OUTDIR_ARG=""
while [ $# -gt 0 ]; do
  case "$1" in
    --keep)     KEEP=1 ;;
    --no-shots) SHOTS=0 ;;
    --lane)     ONLY_LANE="${2:-}"; shift ;;
    --out)      OUTDIR_ARG="${2:-}"; shift ;;
...
XYZ_W="$(nativep "$XYZ")"
# --- scratch directory -----------------------------------------------------
# Everything under TMPBASE is deleted on the way out unless --keep. That makes an
# unvalidated caller-supplied path a foot-gun, so a supplied PROBE_TMP must be
# absolute and must be either missing or empty, and cleanup only ever removes a
# directory THIS run created (OWNS_TMP).
OWNS_TMP=0
if [ -n "${PROBE_TMP:-}" ]; then
  TMPBASE="$PROBE_TMP"
  case "$TMPBASE" in
    /*|[A-Za-z]:/*|[A-Za-z]:\\*) ;;
    *) echo "FATAL: PROBE_TMP must be an absolute path (got '$TMPBASE')" >&2; exit 2 ;;
  esac
  case "$TMPBASE" in
    */) echo "FATAL: PROBE_TMP must not end in a slash (got '$TMPBASE')" >&2; exit 2 ;;
  esac
  if [ -e "$TMPBASE" ]; then
    [ -d "$TMPBASE" ] || { echo "FATAL: PROBE_TMP exists and is not a directory: $TMPBASE" >&2; exit 2; }
    if [ -n "$(ls -A "$TMPBASE" 2>/dev/null)" ]; then
      echo "FATAL: PROBE_TMP is not empty: $TMPBASE" >&2
      echo "       This script deletes its scratch dir at the end. Point PROBE_TMP at a" >&2
      echo "       new or empty directory, or unset it to get a fresh mktemp -d." >&2
      exit 2
    fi
    # existed but empty: adopt without claiming ownership, so cleanup leaves it alone
  else
    mkdir -p "$TMPBASE" || { echo "FATAL: could not create PROBE_TMP: $TMPBASE" >&2; exit 2; }
    OWNS_TMP=1
  fi
else
  TMPBASE="$(mktemp -d 2>/dev/null || mktemp -d -t xyz-probe-c)" \
    || { echo "FATAL: mktemp -d failed" >&2; exit 2; }
  OWNS_TMP=1
fi
TMPBASE_W="$(nativep "$TMPBASE")"
# Recorded immediately and compared before any cleanup, so a later reassignment
# of TMPBASE cannot redirect the rm -rf at something else.
TMPBASE_CREATED="$TMPBASE"

# --- output directory ------------------------------------------------------
# Never $HERE: regenerating evidence must not overwrite committed evidence.
OUTDIR="${OUTDIR_ARG:-${PROBE_OUT:-$TMPBASE/out}}"
case "$OUTDIR" in
  /*|[A-Za-z]:/*|[A-Za-z]:\\*) ;;
  *) OUTDIR="$(cd "$(dirname "$OUTDIR")" 2>/dev/null && pwd)/$(basename "$OUTDIR")" \
       || { echo "FATAL: cannot resolve --out path" >&2; exit 2; } ;;
esac
LOGS="$OUTDIR/logs"; SCREENS="$OUTDIR/screens"
mkdir -p "$LOGS" "$SCREENS" || { echo "FATAL: could not create output dir $OUTDIR" >&2; exit 2; }
...
render() {  # <frame> <title> <rc>
  [ "$SHOTS" -eq 1 ] || return 0
  # NOTE: `html` must NOT be declared in the same `local` statement as `frame`.
  # Bash word-expands every argument to `local` BEFORE the builtin runs, so a
  # $frame there resolves to the CALLER's frame (dynamic scope), not to $1 --
  # and with no caller-scope `frame` at all it aborts outright under `set -u`.
  local frame="$1" title="$2" rc="$3"
  local html="$OUTDIR/html/$frame.html"
  mkdir -p "$OUTDIR/html"
  local badge="#3fb950"
  [ "$rc" != "0" ] && badge="#f85149"
  {
...
mk_fixture() {  # <fixture_dir_native>
  local f="$1"
  # Fixtures are always created under TMPBASE. Assert that before deleting, so a
  # mistyped caller can never hand this an arbitrary directory to wipe.
  case "$f" in
    "$TMPBASE_W"/?*|"$TMPBASE"/?*) ;;
    *) echo "FATAL: refusing to rm -rf '$f' - outside the scratch dir '$TMPBASE_W'" >&2; exit 2 ;;
  esac
  rm -rf "$f" 2>/dev/null
  mkdir -p "$f/bin"
  git -C "$f" init -q
...
} > "$OUTDIR/CONTAINMENT-RESULTS.md"
echo "results table        : $OUTDIR/CONTAINMENT-RESULTS.md"

if [ "$KEEP" -eq 0 ] && [ "$OWNS_TMP" -eq 1 ] && [ -n "$TMPBASE" ] && [ "$TMPBASE" = "$TMPBASE_CREATED" ]; then
  rm -rf "$TMPBASE_W" 2>/dev/null || echo "note: could not remove $TMPBASE_W (NTFS lock) - remove by hand" >&2
elif [ "$KEEP" -eq 0 ] && [ "$OWNS_TMP" -eq 0 ]; then
  echo "fixtures kept        : $TMPBASE_W (PROBE_TMP was supplied and pre-existed - not ours to delete)"
else
