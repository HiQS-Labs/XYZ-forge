...
## T+15→50 min — Kernel probes by hand (see FINDINGS.md / repro.sh)

- Ran P1–P7 live in a throwaway repo `C:\Users\Askyla\Senior_dev\xyz-probe`.
...
- **Guess I made & corrected:** initially I assumed the relay "stopped at exit 3" was a harness bug.
  ...
  harness's no-progress guard is *correct*. Lesson logged in the screenshots README.
...
## T+70→110 min — Reproduction script + screenshots

- Wrote `audit/repro.sh` (one file, all P1–P7). Ran it → 4 invariants confirmed, 2 findings (F1, F5),
  0 info. **Exit 0.** The script is the deliverable that proves every claim.
- Regenerated screenshots with headless Chrome (`file://` via terminal; the browser *tool* blocks
  `file://`/localhost, so capture went through Chrome directly).
...
- `AUDIT-REPORT.md` cites `xyz-screens/*.png` in its deliverables table. **That directory does not exist.**
  A handover that cites evidence which is not there devalues the evidence that is. Repointed at
  `audit/screens/`, which has 48 frames that do exist and an `INDEX.md` mapping each to the log it came from.
