...
AgentChorus (formerly Agent2Agent) is local-first agent coordination for multi-agent development workflows. Install one
...
🌐 **[AgentChorus install & docs](https://agentchorus.dev/)**
...
2. **Ask your agent to start a session.** In plain language, e.g. *"Start an AgentChorus session with Codex to review the new authentication protocol."* The starting agent packages the relevant conversation and repository context into Turn 1.
...
AgentChorus is licensed under the [GNU AGPL-3.0-only](LICENSE), with optional proprietary terms
