...
#
#   bash skills/2-daily/file-xyz-bug/install.sh          # install / repair the symlink
...
SELF_DIR="$(cd -P "$(dirname "$_src")" >/dev/null 2>&1 && pwd)"   # …/skills/2-daily/file-xyz-bug
...
if [ -L "$LINK" ] && [ "$(readlink "$LINK")" = "$SELF_DIR" ]; then
  ...
  if [ -L "$LINK" ] && [ -e "$LINK" ]; then
    ...
    echo "file-xyz-bug: $LINK already points at $(readlink "$LINK") — not replacing a live link." >&2
