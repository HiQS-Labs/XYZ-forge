...
# create task clones (skills/3-weekly/10days, skills/2-daily/marathon-triage); leaving it out meant the one
...
def _tick_binary() -> Optional[str]:
    ...
    if override:
        return override
    for parent in Path(__file__).resolve().parents:  # skills/<tier>/merge-cleanup/scripts/ (GH-744)
        own = parent / "bin" / "tick"
