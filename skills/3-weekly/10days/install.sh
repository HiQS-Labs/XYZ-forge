...
# in one command.
#
#   bash skills/3-weekly/10days/install.sh                    # install / repair the symlink
...
SELF_DIR="$(cd -P "$(dirname "$_src")" >/dev/null 2>&1 && pwd)"   # …/skills/3-weekly/10days
...
if [ -L "$LINK" ] && [ "$(readlink "$LINK")" = "$SELF_DIR" ]; then
  ...
  if [ -L "$LINK" ] && [ -e "$LINK" ]; then
    ...
    echo "10days: $LINK already points at $(readlink "$LINK") — not replacing a live link." >&2
