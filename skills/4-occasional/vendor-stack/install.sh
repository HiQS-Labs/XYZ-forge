...
#
#   bash skills/4-occasional/vendor-stack/install.sh          # install / repair the symlink
...
if [ -L "$LINK" ] && [ "$(readlink "$LINK")" = "$SELF_DIR" ]; then
  ...
  if [ -L "$LINK" ] && [ -e "$LINK" ]; then
    ...
    echo "vendor-stack: $LINK already points at $(readlink "$LINK") — not replacing a live link." >&2
