...
| FD-08 | Undocumented — `skills/1-hourly/relay-xyz/install.sh` only in SKILL.md (chicken-and-egg) | 🟠 | ⬜ | surface in `README.md`/`ROUTER.md` |
...
grep -rlq 'target-root' skills/1-hourly/relay-xyz/SKILL.md relay-automation/QUICKSTART.md 2>/dev/null \
...
grep -rlq 'install.sh' README.md ROUTER.md 2>/dev/null \
  || echo "FD-08 OPEN: skills/1-hourly/relay-xyz/install.sh not surfaced in README/ROUTER"
