...
- Honors explicit user exclusion patterns for checkouts and branches; a numeric value also excludes
  that PR from sequencing before the integration-base gate (e.g. `--exclude 427`).
...
- **Every PR is re-fetched before every decision (E).** `gh pr view` is read fresh per PR. A `mergeable` that is neither `MERGEABLE` nor `CONFLICTING` is polled up to six more times at 15-second intervals; persistent unknown state then stops the run. A non-transient `gh` failure also stops (a PR whose state is unknown is never merged). A TRANSIENT network failure (DNS, refused/timed-out connection, TLS, rate limit) is retried three times first (2s, 4s between calls, GH-623); one that survives the retries DEFERS that PR — named in the end-of-run summary — and the queue continues. A hold label (`hold`, `do not merge`, `blocked`, `wip`) skips the PR (#444). A PR whose base is neither the checked integration branch nor the head of an earlier sequenced stacked PR stops the run.
...
- Executes remote merges in topological sequence (`gh pr merge <PR_NUM> --squash --delete-branch`). Before deleting a base PR's branch, open stacked PRs based on that head are retargeted to the integration branch; if any retarget fails, that merge proceeds without `--delete-branch` and logs the choice. A zero exit is not a landing: the PR is re-queried until it reads `MERGED` with a merge commit (#510 class), else the run fails.
...
- Executes post-merge reconciliation, **gating** (a failure stops the run before emission, commit, push, the next PR, teardown, and symlink pruning; `--reconcile-pr` propagates the same exit):
  - List recent hosted `wave-reconcile.yml` runs without `--branch`/`--commit` filters, then match `headSha` to either the PR head or merge commit. During the listing grace window, any `queued`, `in_progress`, `waiting`, or `requested` run is treated as active even before GitHub exposes its key; once observed, its database id remains authoritative. Poll an active run for at most `MERGE_CLEANUP_HOSTED_WAIT_S` seconds (default 1800); timing out stops the landing rather than racing it locally.
...
# 6. Exclude Active In-Flight Work or a PR (e.g. PR 427 is not sequenced)
