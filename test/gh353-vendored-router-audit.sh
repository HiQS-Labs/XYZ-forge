...
#   3. `router_audit.py --fix` updates ROUTER.md cleanly to declare ROADMAP.md frozen and add ROADMAP-DASHBOARD.md.
#   4. Clean legacy-mode repo reports `ok` (rc=0).
#   5. Drifted legacy-mode repo with phantom ROADMAP-DASHBOARD.md reports `ROUTER DRIFT` and --fix remediates.
#   6. `xyz-sync.sh check` surfaces router drift for registered vendored repositories.
set -uo pipefail
...
- `ROADMAP.md` = LEGACY pointer ledger, frozen since the `ROADMAP_SOURCE=releases` flip — the RELEASES DB (`releases.db` via `releases.sql`) is the source of truth
## Startup sequence
1. Read ROUTER.md
2. Read AGENTS.md
3. Read `ROADMAP-DASHBOARD.md` to find the active effort. (`ROADMAP.md` is the frozen legacy file — do not read it for current state or edit it.)
MD
...
out="$(python3 "$AUDIT_PY" --check "$R_LEG_DRIFT" 2>&1)"; rc=$?
ok "drifted legacy repo reports non-zero exit with --check" "$([ "$rc" -ne 0 ] && echo 0 || echo 1)"
...
# ── 6. xyz-sync.sh check integration ──────────────────────────────────────────────
