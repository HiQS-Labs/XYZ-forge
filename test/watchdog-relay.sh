...
grep -q 'RELAY-TURN"' <<<"$(echo "$out")" && pass "watchdog escalates the stalled RELAY-TURN" || fail "stalled RELAY-TURN not escalated: $out"
grep -q '"agent":"ra"' <<<"$(echo "$out")" && pass "escalation names the stalled actor" || fail "escalation missing actor: $out"
grep -q 'RELAY-TURN-2"' <<<"$(echo "$out")" && fail "healthy RELAY-TURN-2 should NOT escalate" || pass "healthy RELAY-TURN-2 not escalated (no false positive)"
...
grep -q 'reaped 1 claim(s) from ra' <<<"$(echo "$out2")" && pass "--allow-reap reaps the stalled RELAY-TURN (real re-offer, not a stub)" || fail "real reap did not fire: $out2"
grep -qE 'claimer:[[:space:]]+ra' <<<"$(tick_a info RELAY-TURN)" && fail "RELAY-TURN still held by ra after reap" || pass "RELAY-TURN released after reap (orphaned token re-offered)"
grep -qE 'claimer:[[:space:]]+rb' <<<"$(tick_a info RELAY-TURN-2)" && pass "healthy RELAY-TURN-2 (rb) NOT reaped — live claim untouched (never reap a live claim)" || fail "healthy RELAY-TURN-2 wrongly disturbed: $(tick_a info RELAY-TURN-2)"
