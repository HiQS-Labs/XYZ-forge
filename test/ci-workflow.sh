...
# on this same runner and demonstrably finishes inside its cap; a lower boundary cap fails every promotion
# on time, not on code (45 against a 60-92 min suite did). A missing cap on either side fails too.
reconcile_block="$(awk '/^  reconcile:/{f=1} f{print} f && /^  [a-z]/ && !/^  reconcile:/{exit}' "$ROOT/.github/workflows/wave-reconcile.yml")"
boundary_cap="$(sed -nE 's/^[[:space:]]*timeout-minutes:[[:space:]]*([0-9]+)[[:space:]]*$/\1/p' <<<"$boundary_block")"
reconcile_cap="$(sed -nE 's/^[[:space:]]*timeout-minutes:[[:space:]]*([0-9]+)[[:space:]]*$/\1/p' <<<"$reconcile_block")"
