...
const rootCap = src('xyz_work','unavailable',{coverage:'partial',error:'source-unavailable-or-unsupported',roots:[{supported:true,error:'issue-cap'}]});
assert.equal(sourceStatus(rootCap).tone,'partial','a per-root issue cap is not a read failure');
assert.match(sourceStatus(rootCap).help,/issue-cap/);
assert.equal(sourceStatus({...rootCap,roots:[{supported:true,error:'ledger-locked'}]}).tone,'failed');
assert.equal(sourceStatus({...rootCap,roots:[{supported:true,error:'issue-cap'},{supported:true,error:'ledger-locked'}]}).tone,'failed','a failure beside a cap is still red');
