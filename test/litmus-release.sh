...
audit_entry() {
  ...
  if ! printf '%s' "$inv" | /usr/bin/grep -F "\"$gate\"" >/dev/null; then
...
if [ "$MODE" = "mutate" ]; then
  ...
  printf '%s' "$AUDIT_OUT" | /usr/bin/grep '^390 incomplete unaccepted-or-absent-form' >/dev/null \
  ...
  printf '%s' "$AUDIT_OUT" | /usr/bin/grep '^461 incomplete not-registered-in-validate.sh' >/dev/null \
  ...
  printf '%s' "$satisfied" | /usr/bin/grep ' complete ' >/dev/null \
  ...
  printf '%s' "$violated" | /usr/bin/grep 'invariant-unmet' >/dev/null \
