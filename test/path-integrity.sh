...
# SKILL.md under a mktemp root) to exercise the drift guard — fixture literals, not references to
# files in this tree; these names can never exist under the real skills/ (the real skills are named
# per-skill, not demo-greek), so skipping them cannot mask a real path break.
...
# Ballast (added 2026-08-17): test/ballast-release.sh's --mutate-evidence builds a compliant fixture
# manifest member entirely under a mktemp root (`$FIX/test/fixture-gate.sh`,
...
# GH-592 (added 2026-09-13): the same fixture repo names sibling suites `test/gh997-demo.sh` /
# `test/gh998-demo.sh` (resume cases for issues 997/998) and `test/other.sh` (the wrong-suite
