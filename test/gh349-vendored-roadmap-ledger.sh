...
#            non-null backfill, updates on mutation, and exact timestamp preservation across dump/rebuild.
...
# ── 3. Scope 3: Pre-migration fixture, backfill across 9 tables, mutations, dump/rebuild preservation ──
R_PRE="$(mkrepo pre_mig_repo)"
require_fixture "$R_PRE" "pre-mig fixture repo"

# Build a pre-007 fixture (v6 schema migrations)
...
# Run migrate
python3 "$APP" --root "$R_PRE" migrate >/dev/null
...
for tbl in "${TABLES[@]}"; do
  ...
  if [ "$null_count" -ne 0 ]; then
    all_non_null=0
    echo "table $tbl has $null_count NULL updated_at rows after migration" >&2
  fi
done
ok "all 9 tables have 100% non-NULL updated_at backfill after migration 007" "$(is "$all_non_null" "1"; echo $?)"
...
# Test dump and rebuild exact timestamp preservation
python3 "$APP" --root "$R_PRE" check >/dev/null
PRE_REBUILD_TIMES="$(sqlite3 "$R_PRE/releases.db" "SELECT updated_at FROM releases ORDER BY id;")"
python3 "$APP" --root "$R_PRE" check --rebuild >/dev/null
POST_REBUILD_TIMES="$(sqlite3 "$R_PRE/releases.db" "SELECT updated_at FROM releases ORDER BY id;")"
ok "check --rebuild preserves exact updated_at values" "$(is "$PRE_REBUILD_TIMES" "$POST_REBUILD_TIMES"; echo $?)"
...
# Test ovr orphan (ovr without rated) refusal
cat > "$R/ROADMAP.md" <<'MD'
# Foreign Org Roadmap
## Ledger
### Active
