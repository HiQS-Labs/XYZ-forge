...
python3 "$EXPORTER" --db "$DB" --json > "$JSON_OUT"
rc=$?
ok "--json exits 0" "$rc"
...
python3 "$EXPORTER" --db "$DB" --out "$NORMAL_OUT_DIR" >/dev/null 2>&1
...
echo "-- testing drift guard (GH-568: retired in favor of releases.db) --"
python3 "$EXPORTER" --db "$DB" --check-drift > "$WORK/drift.txt" 2>&1
rc=$?
if [ "$rc" -eq 0 ] && grep -qi "authoritative" "$WORK/drift.txt"; then
  ok "drift guard exits 0 and reports releases.db is authoritative" 0
else
  echo "Output was:"
  cat "$WORK/drift.txt"
  ok "drift guard check failed" 1
fi

# Ensure --json doesn't write any files
python3 "$EXPORTER" --db "$DB" --json --out "$WORK/timeline" > /dev/null
