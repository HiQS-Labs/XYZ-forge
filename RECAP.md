...
**Correction (2026-06-21, append-only):** the "`GH Repos` vs `GitHub-Repos` clone split" above was an auditor misread — `GitHub-Repos` was a **symlink alias → `GH Repos`** (one physical directory, verified `pwd -P`), not a second clone, so there was no divergence (the byte-identical `.git` should have tipped me off). Per operator instruction the alias was deleted as a confusing crutch; the 20 `~/.claude/skills` symlinks that routed through it were first repointed to `GH Repos` and verified resolving, then the alias was removed. Net remaining deferred hygiene = just the two genuinely-dangling `consult`/`wpcc` symlinks.
