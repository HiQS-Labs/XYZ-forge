...
**One gateway, one approval authority.** Use `python3 utils/py/gate_inventory.py admission` for coverage decisions; [TEST-ADMISSION.md](docs/testing/TEST-ADMISSION.md) owns command usage and rollout. The oracle/writer records decisions; the human operator approves them. Agents may propose coverage, never approve their own proposal or use the operator's credentials to submit approval.

- **Disclose before expanding coverage.** For executable changes, record `reuse`, `extend`, `add`, or `no-add`, the behavior protected, existing coverage/overlap, reason, rejecting evidence, cost and tracking issue. Prefer useful existing coverage; disclose additions inside existing suites as well as new files. Do not silently create tests to satisfy a ritual or increase a count.
- **Route every test change through the writer.** Additions, edits, deletions, renames, parameterized-case changes and changes to relevant helpers or collection configuration require a current decision covering the exact content. Direct edits are unapproved proposals until admitted; hand-editing catalog metadata or regenerating a proposal cannot authorize them.
- **Only the authority records approved budgets.** Each admitted suite must have an explicit maximum in a named counting unit, an inventory digest, content fingerprints and approval provenance tied to the reviewed revision. Agents must not raise ceilings, replace approved fingerprints, backfill approval or change the verifier/baseline to make an unauthorized change pass. A generated proposal is never an approved record.
- **A count alone is insufficient.** Use runner-collected cases where reliable, distinguish them from suite/file counts, and report unsupported counts as unknown. Content changes still require approval below the ceiling; do not offset an unauthorized addition by deleting another test or hide growth inside parameters/helpers. Freezing legacy coverage for growth control does not certify its value or authorize deletion.
- **Refuse unauthorized drift.** The enforced gate must fail for an exceeded ceiling, missing/stale approval, changed approved content, or a hand-edited authority record. Correct the proposal and obtain renewed operator approval; never auto-accept drift, bypass the boundary, or weaken the check to proceed. Exact authorized content is the contract; the editor used to produce it is not proof of authorization.

**Current enforcement status (PR #811):** proposal validation is implemented on the task branch; authority-written ceilings/content sealing are planned, and native protection/credential separation are not activated. These behavioral rules apply now, but agents must not claim that the pending machine checks already enforce them. Keep unapproved test changes out of landed work and follow the runbook's operator handoff.
