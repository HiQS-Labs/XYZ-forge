...
- **The "26 Tool" Rule:** When an agent or runner requires $\le 25$ distinct tools and shallow execution chains, structured JSON tool calling is supported. When scaling to $> 25$ tools or multi-step sequential chains, programmatic script-mode execution (generating and running standalone Python scripts via `utils/py/script_runner.py`) is authoritative.
- **Quantitative Decision Rule:** Code-mode is recommended over JSON function calling whenever:
  $$\text{Cost}_{\text{Code}} < \text{Cost}_{\text{JSON}} \quad \text{OR} \quad \text{Latency}_{\text{Code}} < 0.60 \times \text{Latency}_{\text{JSON}}$$
  at $\ge 95\%$ task accuracy (empirical baselines recorded in `test/baselines/gh94-tool-calling-benchmarks.jsonl`).
- **Containment Invariant:** All programmatic execution must run inside an isolated process group (`setsid`) with PGID-targeted cleanup (`SIGTERM`/`SIGKILL`) and resolved use-boundary path containment under `$WORK` (verified by `test/synthetic/gh94-containment-invariants.sh`).
...
|---|---|---|---|
| **2026-08-20** | Programmatic Tool Calling (GH-94) | Policy & benchmark baseline recorded | Integrated `utils/py/script_runner.py` with PGID timeout containment; instrumented `utils/ate/scripts/run_variations.py` with structured telemetry (`error_log.jsonl`); verified synthetic suites `gh94-script-serialization.sh` and `gh94-containment-invariants.sh` (7/7 PASS); committed benchmark receipts across 5–100 tool densities in `test/baselines/gh94-tool-calling-benchmarks.jsonl`. |
