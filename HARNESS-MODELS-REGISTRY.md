...
|---|---|:---:|---|:---:|---|---|
| DeepSeek Harness (`dsh`) | `openrouter/deepseek/deepseek-v4-pro` | Cloud | Autonomous Headless Builder & Task Solver | **A** | `DSH_PERMISSION_MODE=danger-full-access node .../apps/cli/lib/bin.js --profile headless --patch <overlay.cordis.yml> "<task>"`. Evaluated across 3 real repository bugs in isolated clone. | [GH-148 umbrella tracking](https://github.com/HiQS-Suite/XYZ-forge/issues/148); successfully resolved GH-142 (`compile_issue.py` missing import and error exit code propagation), GH-68 (`HARNESS-MODELS-REGISTRY.md` Table 1 schema alignment), and GH-65 (`test/` portable 3-tier artifact hashing) with zero human intervention and 15–40s wall-clock latency. |
