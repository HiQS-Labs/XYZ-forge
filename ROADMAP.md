...
- **GH-11 · relay-xyz cross-repo targeting** 🟢 — Ask 1 `--target-root` **flag** increment landed (agy-built, Codex-approved, `validate.sh` 35/35; inert until wired). Remaining: kernel wiring (`relay-turn-lib.sh` consumes `RELAY_TARGET_ROOT`), the empty-string `[Nit]`, + surface consult & doc fixes (Asks 2–5). → [GH-11-CROSS-REPO-TARGETING.md](PROJECT/2-WORKING/GH-11-CROSS-REPO-TARGETING.md)
