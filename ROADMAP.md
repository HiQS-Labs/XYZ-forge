...
### Queue / parked intake
- **GH-103 · technical spike: RELEASES SQLite → timeline-ui ledger viewer** 🆕 **queued 2026-08-20; spike BUILT same day on `feat/gh103-timeline-ledger-spike` (standalone clone `XYZ-forge-gh103-timeline-spike`)** — read-only exporter `utils/timeline/export_timeline.py` projects `releases.db` (releases + manifest + GH-69 roadmap shadow) into timeline-ui's `data.json` contract and bakes a self-contained static page (`temp/timeline/index.html`, opens from `file://`); adapted viewer template at `utils/timeline/ledger.html`. Zero DB writes (SQLite `mode=ro`). Prototype for the queued #75 `releases dashboard` verb. → [GH-103-TIMELINE-LEDGER-SPIKE.md](PROJECT/1-INBOX/GH-103-TIMELINE-LEDGER-SPIKE.md) · [#103](https://github.com/HiQS-Suite/XYZ-forge/issues/103)
