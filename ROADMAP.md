...
### In progress
- **GH-111 · retire manifest FREEZE; tasks and marathons are DIALED IN to exactly one release, as a database state** 🚧 **Phases A, B and C BUILT 2026-08-20 on `feat/gh111-phase-a-dialed-in`** — membership becomes machine-checkable instead of a sentence someone remembered to write: three states (`dialed_in`/`shipped`/`cut`), exclusivity by a partial unique index over ACTIVE membership only, a real `releases migrate` upgrade path where none existed, and BASELINES replacing what the freeze used to buy (progress against the live manifest, growth against the kickoff count). Closes #109 (the viewer asserted marathon membership the data never claimed) and #110 (`shipped` was a state nothing could write). rated 85/75/70/35. → [GH-111-DIALED-IN.md](PROJECT/2-WORKING/GH-111-DIALED-IN.md) · [#111](https://github.com/HiQS-Suite/XYZ-forge/issues/111)
- **GH-108 · pri/sev/appeal/effort — the canonical task rating system (calc sum + operator override)** 🚧 **all four phases BUILT 2026-08-20 on `feat/gh111-phase-a-dialed-in`** — one scoring vocabulary for every task in the ledger, oriented so a single glance answers what should go to the front of the line: four axes 1-100 where higher is always better (effort scores cheapness), a combined 4-400 `calc` derived and never stored, and an `ovr` override that pins anything visible. Authored as `rated N/N/N/N` on a ROADMAP entry; read via `bash utils/leaderboard.sh` or LEADERBOARD.html. rated 80/50/75/40. → [GH-108-RATING-SYSTEM.md](PROJECT/2-WORKING/GH-108-RATING-SYSTEM.md) · [#108](https://github.com/HiQS-Suite/XYZ-forge/issues/108)
- **Execution checklist for GH-111 + GH-108** 🚧 **complete 2026-08-20** — the running list covering every deliverable of the two plans above, in dependency order, so a stall at any point left the next move unambiguous. Kept until both plans move to 3-COMPLETED. rated 40/20/35/95. → [GH-108-GH-111-EXECUTION-TODO.md](PROJECT/2-WORKING/GH-108-GH-111-EXECUTION-TODO.md) · [#111](https://github.com/HiQS-Suite/XYZ-forge/issues/111)
