...
- **GH-94 · research: programmatic tool calling & code-mode execution for harnesses, telemetry, and containment** 🚧 **active 2026-08-19** — evaluate the architectural shift from JSON schemas to code-mode tool execution: the "26 tool" tipping point in custom adapters (`utils/ate/`, Pi, SmallCode), single-turn diagnostic chaining in marathon review loops, telemetry extraction, and containment hardening against script serialization and escape defects. → [GH-94-PROGRAMMATIC-TOOL-CALLING.md](PROJECT/2-WORKING/GH-94-PROGRAMMATIC-TOOL-CALLING.md) · [#94](https://github.com/HiQS-Suite/XYZ-forge/issues/94)
- **GH-101 · Feasibility Study: Promoting Programmatic Script Runner (`script_runner.py`) into Core Relay & Consult Runtimes** 🆕 **queued 2026-08-20 (Test 1 of 3)** — evaluate the feasibility, safety, and performance of promoting `utils/py/script_runner.py` into core XYZ runtime workflows (`consult.py`, `.relay-scratch/` diagnostics, and programmatic agent turns) across a 3-test qualification ladder. → [GH-101-FEASIBILITY-STUDY-SCRIPT-RUNNER.md](PROJECT/1-INBOX/GH-101-FEASIBILITY-STUDY-SCRIPT-RUNNER.md) · [#101](https://github.com/HiQS-Suite/XYZ-forge/issues/101)
- **GH-102 · Unify Telemetry Schema & Inspection Tooling Across Fuzzing and ATE** 🆕 **queued 2026-08-20** — standardize the data contract (`schema_version: "1.0"`), shared telemetry fields, and universal inspection CLI (`checkin.py`) across deterministic synthetic fuzzing (`utils/fuzzing/`) and stochastic LLM variation benchmarks (`utils/ate/`), keeping execution engines decoupled. → [GH-102-UNIFIED-TELEMETRY-TOOLING.md](PROJECT/1-INBOX/GH-102-UNIFIED-TELEMETRY-TOOLING.md) · [#102](https://github.com/HiQS-Suite/XYZ-forge/issues/102)
### In progress
