...
- **GH-21 · relay quality gate — independent post-generation validator** 🟡 — **active (promoted 2026-06-25)**: add a deterministic structural quality gate to the relay protocol independent of the producing agent. Three phases: (1) `bin/validate-relay-block` hooked into `bin/tick release`/`done` + new `exit 8`; (2) `test/relay-self-sufficiency.sh` for TAKE YOUR TURN self-sufficiency; (3) opt-in `--consult-verify` on `relay-drive.sh`. QA-reviewed by claude-b (FAIL → applied) then agy (PASS → promoted). → [GH-21-RELAY-QUALITY-GATE.md](PROJECT/2-WORKING/GH-21-RELAY-QUALITY-GATE.md) · [#21](https://github.com/Claude-AI-Tools-Ventura-County/xyz-3-agents-swarm/issues/21)
- **GH-20 · agy first-class footing in live relay docs** 🟡 — **active (started 2026-06-25)**: remove the remaining live doc/skill bias that presents Codex as the primary headless Path-A agent and agy as a swap-in. Scope: front door, canonical bring-up, relay skill, and adjacent live discovery labels only; keep runtime differences explicit where they are real. → [GH-20-AGY-FIRST-CLASS-FOOTING.md](PROJECT/2-WORKING/GH-20-AGY-FIRST-CLASS-FOOTING.md)
