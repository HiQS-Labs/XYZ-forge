...
3. **[#59](https://github.com/HiQS-Suite/XYZ-forge/issues/59) — find out why hosted CI fires on nothing**, then narrow triggers to push/merge on `development` + `main` and wire the required check behind `main`'s new branch protection. **Re-confirmed 2026-08-21 with seven hypotheses ruled out** (billing, workflow state, fork status, base filter, workflow-absent-from-head, fork approval); what remains is Settings → Actions → General by eye, since the deciding endpoints need `admin:org`. Evidence in the issue; both this and the advisory-green trap below are codified in `.github/workflows/ci.yml`'s header so they stop being rediscovered. rated 75/60/40/70.
