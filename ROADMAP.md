...
- **GH-12 · `tick` silently no-ops from a foreign CWD** 🟢 — **both fixes landed 2026-06-22**: `bin/tick` now echoes the resolved root and **refuses** a coordination-mutation verb when the root was *inferred* (not pinned via `TICK_REPO_ROOT`) and the repo has no `.tick/events` — closing the silent succeed-as-noop that stalled the relay handoff; `relay-automation/poll.sh` dispatches a bare executable path directly (space-safe, mirrors `relay-drive.sh`). New `test/tick-foreign-cwd.sh` + spaced-path `poll-driver.sh` case; pkg regenerated; **`validate.sh` 37/37**. Remaining: operator review → commit/push → close issue #12. → [GH-12-TICK-FOREIGN-CWD-SILENT-NOOP.md](PROJECT/2-WORKING/GH-12-TICK-FOREIGN-CWD-SILENT-NOOP.md)
