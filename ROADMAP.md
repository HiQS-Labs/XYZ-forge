...
- **GH-144 · Agent2Agent 3+ participant onboarding + read-only status quick wins** 🚧 **BUILT 2026-08-21 in standalone full clone; PR pending** — `start --agents N` prints every non-initiator invitation and seat-agnostic `status --id` reports roster/turn/advisory doorbells without writing; no protocol, storage, routing, or transport expansion. Focused suite 129/0 with recorded pre-fix red control. rated 55/30/80/90. → [GH-144-AGENT2AGENT-3PLUS-UX.md](PROJECT/2-WORKING/GH-144-AGENT2AGENT-3PLUS-UX.md) · [#144](https://github.com/HiQS-Suite/XYZ-forge/issues/144)
