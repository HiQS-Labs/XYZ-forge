...
### Queue / parked intake
- **GH-24 · relay telemetry extractor** 🟡 — **intake 2026-06-25**: on-demand ETL script that reads `relay-system/<date>/*.md` for a configurable `--from`/`--to` date range and outputs `relay-system/combined/aggregated-FROM-to-TO.json` matching the focus5float stoplight schema `{ health, title, description, updatedAt }`. Health derived from STATUS: + VERDICT: fields. Script authored: `utils/telemetry/extract-relay-telemetry.sh`. → [GH-24-RELAY-TELEMETRY-EXTRACTOR.md](PROJECT/1-INBOX/GH-24-RELAY-TELEMETRY-EXTRACTOR.md) · [#24](https://github.com/Claude-AI-Tools-Ventura-County/xyz-3-agents-swarm/issues/24)
