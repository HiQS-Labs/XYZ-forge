...
- **GH-18 · cross-repo driven-relay friction** ⏸️ — **verification + docs done 2026-06-24, code PAUSED for review**: field-validated punch-list from a real cross-repo Codex review (thread/artifact in `rebalance-OS`, harness here via `--target-root`). Phase 0 verification reproduced #1 (`RELAY-TURN` singleton token collision), #2 (`--relay-file` resolved vs CWD not `--target-root`), #5 (by-design `Escalated` reads as a stall) as real code bugs; #3 (foreign `.tick`) found **largely stale** for driven runs (mitigated by [codex-turn.sh:57](relay-automation/codex-turn.sh#L57)) → doc-only; #4 doc-only. Phase 1 docs shipped (QUICKSTART: per-relay token id + cross-repo recipe). Code phase (#1b token default, #2 path resolution, #5 escalation oracle) **awaiting operator go-ahead — no code touched**. Child of GH-16. → [GH-18-CROSS-REPO-RELAY-FRICTION.md](PROJECT/2-WORKING/GH-18-CROSS-REPO-RELAY-FRICTION.md) · [#18](https://github.com/Claude-AI-Tools-Ventura-County/xyz-3-agents-swarm/issues/18)
