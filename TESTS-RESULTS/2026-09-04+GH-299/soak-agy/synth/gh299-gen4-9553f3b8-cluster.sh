#!/usr/bin/env bash
# GH-299 Gen 4 synthesized reproducer — cluster 9553f3b817475f28 (5 counterexample(s), phases fuzz, verdicts fail) (Synthesized by utils/py/repro_builder.py - GH-155 Phase 3)
# Synthesized by utils/py/repro_synth.py (GH-299 Phase 4).
# cluster=9553f3b817475f28 members=5 original_argv=python3 utils/py/adaptive_ate.py --mode coverage --grid utils/
...
RC=0
python3 utils/py/adaptive_ate.py --mode coverage --grid utils/ > "$STDOUT_FILE" 2> "$STDERR_FILE" || RC=$?
...
if ! grep -q '~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^' <<<"$STDERR" && ! grep -q '~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^' <<<"$STDOUT"; then
  echo "FAIL: Expected substring '~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^' not found in stdout or stderr"
