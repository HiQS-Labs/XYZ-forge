...
def main():
    ...
    request={'model':'gemma-4-12b-it-mlx',
        ...
        'stream':True,'store':False,'temperature':0,'max_output_tokens':6000}
    data=json.dumps(request,ensure_ascii=False).encode()
    with (base/(run+'-request.json')).open('xb') as f:
        f.write(data);f.flush();os.fsync(f.fileno())
    receipt={'schema':'needle13/gemma-local-spike@1','run':run,'status':'pending',
    ...
    print('provider: LM Studio localhost / MLX',flush=True)
