...
def main():
    ...
    ref=json.loads(pathlib.Path(os.environ['ANALYST_SOURCE_REFERENCE']).read_text())
    ...
    request={'model':'google/gemma-4-31b-qat',
        ...
        'input':pathlib.Path(os.environ['ANALYST_PROMPT_FILE']).read_text()+'\n\n'+source.decode(),'integrations':[],
    ...
    receipt={'schema':'needle13/git-analyst-gemma31-local-spike@1','run':run,'status':'pending',
