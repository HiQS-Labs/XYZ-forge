...
def main():
 ...
 flags=['--model','gemini-3.7-flash-high','--effort','high','--sandbox','--mode','plan','--disable-slash-commands','--output-format','json','--print-timeout','14m','--log-file',str(b/(run+'-runtime.log'))]
 receipt={'schema':'needle13/agy37-cli-spike@1','run':run,'status':'pending','started_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'requested_model':'gemini-3.7-flash-high','requested_effort':'high','prompt_sha256':hashlib.sha256(prompt.encode()).hexdigest(),'source_packet_sha256':ref['sha256'],'flags':flags,'cwd':os.getcwd(),'wall_cap_seconds':900,'cli_print_timeout_seconds':840,'system_prompt_is_user_prompt_prefix':True,'temperature':None,'max_output_tokens':None,'backend_model_attestation':None}
