...
def main():
 ...
 b=pathlib.Path(os.environ['LOCAL_SPIKE_ROOT']).resolve(strict=True);run=os.environ['LOCAL_SPIKE_RUN'];assert run in ('high-r1','high-r2','xhigh-r1','xhigh-r2');effort=run.split('-')[0]
 ...
 flags=['exec','-m','gpt-5.3-codex-spark','-c','model_reasoning_effort="'+effort+'"','-c','approval_policy="never"','-s','read-only','--ephemeral','--ignore-user-config','--json','--color','never','-o',str(b/(run+'-answer.md')),'-']
 r={'schema':'needle13/spark-cli-spike@1','run':run,'status':'pending','started_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'requested_model':'gpt-5.3-codex-spark','requested_effort':effort,'prompt_sha256':hashlib.sha256(prompt.encode()).hexdigest(),'source_packet_sha256':ref['sha256'],'flags':flags,'cwd':os.getcwd(),'wall_cap_seconds':900,'subprocess_timeout_seconds':870,'system_prompt_is_user_prefix':True,'temperature':None,'max_output_tokens':None,'backend_model_attestation':None};save(b/(run+'-receipt.json'),r);start=time.perf_counter();stop=threading.Event()
 def beat():
