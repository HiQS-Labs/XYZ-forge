...
- **Layout C**: `layout-c.html?repo=ltvera` — issues within the selected repo, using the same tall cards and gestures. Each issue shows its agents, last-hour activity, next action and lane-linked PRs. Fewer cards are centered. X zooms out to the same repo and carousel position in B. Escape clears an issue spotlight first, then zooms out on the next press. Workspace folders are explicitly shared repo context; issue ownership is unknown.
...
**Escape goes up one level per press:** issue spotlight → issue cards (C) → repo spotlight (B) → repo cards (B) → saved Layout A. Holding Escape does not repeat navigation.
