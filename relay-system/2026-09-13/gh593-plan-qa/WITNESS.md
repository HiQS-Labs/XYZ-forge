...
| F. hand-mutated | A's row edited to `~~#…~~ → solved` | — | — | rejected by rule text: *"solved — the only state that is struck through, and only on quiet 2/2 and a fix merged PR"* — A has neither | — |
...
Rendered a draft umbrella body with the block, then deleted the fenced block → §6 rule: *"A body without the fenced Cluster signature block, or with any key missing, is a template violation — do not file it."* Deleted only the `signals:` key → same rule fires.
