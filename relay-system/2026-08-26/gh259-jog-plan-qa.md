...
5. **Fire-time contract promotion & probe hardening (Finding 5):** Resolved the `1-INBOX` vs `swarm-preflight` contract barrier: `jog run` promotes `1-INBOX` captures to `2-WORKING/` upon pop. To eliminate the weak link of auto-scaffolded probes falsely passing on garbage, Phase 1 requires probe linting and defaults to interactive operator confirmation (or parks if unattended without prior probe definitions) before running preflight; mapped exits (`ready` -> fire, `already-landed` -> drop with receipt, `not-ready` -> park/prompt).
6. **Landing confirmation boundary & re-anchoring (Finding 6):** Resolved the conflict between serial safety rationale and landing boundary: `jog run` **defaults to pausing at each landing boundary for operator confirmation** before merging PRs into `development` and advancing, with `--auto-merge` as an explicit opt-in flag for unattended runs. Same-seam overlapping tasks apply `merge-before-advance` re-anchoring; stacked branches deferred as explicit future opt-in.
