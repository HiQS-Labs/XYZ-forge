---
name: shakedown
description: Systematically test and harden any other Claude Code skill that calls bundled bash/shell scripts, especially when those scripts are found inconsistently or reported "not found" across sessions, repos, or installs. Use whenever a skill's scripts work in one session but fail in another, when script paths resolve against the wrong directory, or when the user says "test this skill", "shake out the path bugs", "why can't Claude find my skill's scripts", "the bash script isn't being located", "harden this skill before I ship it", "audit the skill's script paths", or points at a script-calling skill that behaves unreliably. Runs a read-only static path audit plus a live scenario matrix (foreign working dir, nested dirs, spaces-in-path, project vs user install, stripped execute bit), reproduces the discovery failure, verifies a proposed fix, then writes a graded report and a ready-to-apply SKILL.md patch into a central SHAKEDOWN/ folder so findings never have to be copy-pasted out of chat.
---
...
- The **only** thing written to the user's repo is the report file under `SHAKEDOWN/` — and by default that report *proposes* a patch rather than applying it (see Defaults).

## Locate this skill, then call its scripts by absolute path

This skill practices what it checks. When it triggers, Claude Code provides this skill's directory — run the bundled scripts by that **absolute** path, never by a CWD-relative path. If the directory isn't obvious, discover it once:

```bash
SK=$(dirname "$(find "$HOME/.claude/skills" ".claude/skills" -path '*shakedown/SKILL.md' 2>/dev/null | head -n1)")
```
...
**Verdict:** <🚧 path bug reproduced | ⚠️ warnings only | ✅ clean> — <one line>
...
- **Anchor the verdict to the worst load-bearing finding**, not an average. One reproduced 🚧 discovery failure makes the whole skill's verdict 🚧, even if everything else passes.
