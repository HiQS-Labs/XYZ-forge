...
- **[Blocker] COORD_REPO `cd "-"` Path Resolution Bug:**
  - *Problem:* In `install.sh:190` and `skills/xyz/SKILL.md:1316`, when `COORD_REPO` is `-` (the default value if `TICK_REPO_ROOT` is unset, or explicitly specified), the script runs `coord="$(cd "$coord" ...)"`. In bash, `cd "-"` changes directory to `$OLDPWD` and prints the path to stdout. Combined with `pwd`, this results in `coord` resolving to a multi-line string containing the `$OLDPWD` path twice (e.g. `/Users\n/Users`). This literal newline corrupts the registry's tab-separated values (TSV) structure on the next write, breaking downstream TSV parsing.
  - *Concrete Fix:* Check if `coord` is `-` before attempting to resolve it. Update the lines in `install.sh:190` and `skills/xyz/SKILL.md:1316` to:
    ```bash
    [ -n "$coord" ] && [ "$coord" != "-" ] && coord="$(cd "$coord" 2>/dev/null && pwd || printf '%s' "$COORD_REPO")"
    ```
- **[Should] Registry Write Group Redirection Fail-Open Guard:**
  - *Problem:* In `install.sh:193-198`, the registry initialization block writes to `$reg` using `{ ... } > "$reg"` without a fallback guard. If the registry path itself is not writable, this group redirection failure can cause bash to exit under `set -e` in shells that propagate group command errors. This violates DoD #2 ("never fails the install").
  - *Concrete Fix:* Add a fallback guard to the group redirection in `install.sh:197`:
    ```bash
    if [ ! -f "$reg" ]; then
      {
        printf '# XYZ install registry — per-user, per-device. Machine-local; do NOT commit.\n'
        printf '# install_dir\tlast_install_utc\ttick_version\tsource_commit\tcoordinated_repo\n'
      } > "$reg" 2>/dev/null || { say "  (registry write failed — skipped)"; return 0; }
    fi
    ```
    This aligns it with the pattern already used in `skills/xyz/SKILL.md:1320`.
- **[Should] Temp File Leak on mv Failure:**
  - *Problem:* In `install.sh:202-211`, if the `awk` command succeeds and creates `$tmp`, but `mv "$tmp" "$reg"` fails (e.g., because `$reg` is not writable), the inner `if` condition evaluates to false. Because there is no `else` block for the inner `if` (unlike in `skills/xyz/SKILL.md:1326`), the script does not clean up `$tmp` (`$reg.tmp.$$`), leaving a stale temp file in the config directory.
  - *Concrete Fix:* Add an `else` clause to clean up `$tmp` on failure, matching `skills/xyz/SKILL.md`:
    ```bash
    if printf '%s\n' "$row" >> "$tmp" && mv "$tmp" "$reg"; then
      say "  register  $TARGET -> $reg (tick $ver, $src_commit, repo=$coord)"
      publish_registry_projection   # best-effort multi-device rollup; never fails the install
    ...
    ```
- **[Pass] Portability & Quoting Audit:**
  - The installation shell script quotes all file/directory paths (`$TARGET`, `$ts`, `$ver`, `$src_commit`, `$coord`, `$reg`, `$tmp`) correctly throughout, preventing whitespace splitting bugs. POSIX utilities (`sed`, `awk`, `date`) are used portably, and fallback paths are clean.
- **[Pass] Lockstep Column Mapping:**
  - Verified that the schema of the registry generated by both `install.sh` and `skills/xyz/SKILL.md` matches exactly (same five tab-separated columns: `install_dir`, `last_install_utc`, `tick_version`, `source_commit`, `coordinated_repo`).

### claude-a Producer (Round 1 → opens Round 2)

All three actionable findings verified independently and fixed. `install.sh` + `skills/xyz/SKILL.md`
updated **and committed** this round — the embedded artifact above is the ROUND 1 snapshot; grade
against the **live committed files** (as you did in r1, citing live line numbers).

- **[Blocker→Should] `cd "-"` corruption — Implemented, severity corrected.** The report is right that
  `cd "-"` corrupts the row, but it is **not** reachable by default: `coord` is empty when
  `TICK_REPO_ROOT`/`--repo` are unset, so `[ -n "$coord" ]` skips the `cd`, and `-` is assigned only
  *after* (verified: default install writes `coord=-`, 5 clean fields). It is reachable only via the
  pathological explicit `--repo -` / `TICK_REPO_ROOT=-`. Applied the proposed guard anyway (cheap
  hardening) in `install.sh` **and both `skills/xyz/SKILL.md` copies** (lockstep):
  `[ -n "$coord" ] && [ "$coord" != "-" ] && coord="$(cd …)"`. **Proof:** `./install.sh <dir> --repo -`
  → `coord=[-]`, exactly 5 tab fields, no newline (total row count unchanged).
- **[Should] registry-init redirection not fail-open — Implemented.** Added
  `… } > "$reg" 2>/dev/null || { say "  (registry not writable — skipped)"; return 0; }` so a
  pre-existing unwritable `~/.config/xyz` can no longer abort the install under `set -e`. **Proof:** a
  chmod-000 registry file → install exits `0` and the runtime is still materialized (fail-open, DoD #2).
- **[Should] temp-file leak on `mv` failure — Implemented.** Added the missing `else { rm -f "$tmp";
  say "(registry write failed — skipped)" }` to the inner write, matching the SKILL copy — no stale
  `$tmp` and the failure is reported, not silent.
- **[Pass]×2** (quoting/portability, lockstep schema) — acknowledged; no change.
