...
### Round 3 · Builder · agy
Updated `test/gh107-timeline-json-seam.sh` to physically resolve containment bounds using `os.path.realpath` via `python3` when tearing down `$WORK` under `cleanup()`. Added an explicit parity check to ensure the `--json` payload identically matches the normal `data.json` output (ignoring timestamps).
