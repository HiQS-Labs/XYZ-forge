# RELAY · Review swe-diagram skill r2
<!--
...
6. **Commit only the relay file** (`relay(swe-diagram-review-r2): <role> r<N>`); no push. **Stop** and report one line.

## Setup
- Artifact under review: **swe-diagram-review-bundle-r2.md** (embedded below — read it here).
- Reviewer: codex   ·   Producer: claude-a
- Started: 2026-07-04

### Artifact — swe-diagram-review-bundle-r2.md
````
# Review bundle (round 2) — swe-diagram skill, after applying round-1 fixes

Round 1 (Codex) requested changes. Applied since then:
- Layout now condenses cycle back-edges (DFS) before longest-path ranking, so bidirectional
  pairs stay in adjacent columns instead of drifting apart.
- build-diagram.sh escapes '<' to \u003c so a '</script>' inside any spec string cannot close
  the embedding <script> tag early.
- renderer.js now honors spec.groups by rendering the human label via an id->label map.
- Removed a U+FFFF sort sentinel (now ASCII-safe) and an en-dash in SKILL.md.
Declined: 'missing README update' (both READMEs were in fact updated; only the 4 skill files
were bundled for review) and the em-dash/arrow ASCII nits (AGENTS.md explicitly allows em-dashes;
the arrow glyph is used throughout the repo's own README).

Please re-review for correctness and confirm the round-1 Blocker (cycle layout) is resolved.
...
html = html.replace("__DIAGRAM_JSON__", diagram_json)
...
- [Pass] The round-1 blocker is resolved. `layout()` now drops DFS-detected back-edges before longest-path ranking, so a bidirectional pair no longer gets artificially stretched across extra columns just because the reverse edge feeds the rank pass.
- [Should] `renderer.js` still hard-codes `nodeHeight()` to `56` or `74`, but the DOM nodes can grow taller when `label` or `tech` wraps. That makes column spacing and edge anchors use stale heights, so longer component names can overlap the next node and route edges through the wrong vertical midpoint. Concrete fix: either force single-line truncation in CSS, or measure each rendered node's actual height before finalizing `pos[]` and drawing edges.
- [Should] `build-diagram.sh` escapes the embedded JSON but still injects `spec.title` raw into `<title>__TITLE__</title>`. A title containing `<`, `&`, or `</title>` can still corrupt the head or inject markup even after the `</script>` fix. Concrete fix: HTML-escape the title substitution before `html.replace("__TITLE__", ...)`, e.g. with `html.escape(...)`.

**Verdict:** Changes requested
VERDICT: FAIL
Basis: Static review of the embedded round-2 `swe-diagram` bundle against the relay brief and the skill's stated output contract; no artifact edits, source edits, or repo-wide gates were run because this turn is review-only and relay-file-scoped.
