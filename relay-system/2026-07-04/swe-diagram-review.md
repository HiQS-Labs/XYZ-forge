# RELAY · Review swe-diagram skill
<!--
...
6. **Commit only the relay file** (`relay(swe-diagram-review): <role> r<N>`); no push. **Stop** and report one line.

## Setup
- Artifact under review: **swe-diagram-review-bundle.md** (embedded below — read it here).
- Reviewer: codex   ·   Producer: claude-a
- Started: 2026-07-04

### Artifact — swe-diagram-review-bundle.md
````
# Review bundle — swe-diagram skill (giant-brains-claude-skills/utils/swe-diagram)

A new Claude Code skill. It draws a system architecture diagram for a repo, preferring the repo's
existing rendered knowledge (graphify output, codebase-memory graph, ask_self RAG, arch docs) over
grepping raw code. Deliverables: a JSON spec + a self-contained interactive xyflow-style HTML file
rendered by a bundled dependency-free JS renderer. Review for correctness, robustness, and whether
it honors the repo's authoring conventions (see AGENTS.md: brevity, ASCII punctuation, triggers in
description, counter-example, README updated in same change).
...
(function () {
  ...
  window.renderDiagram = function (spec, mount) {
    ...
    (spec.nodes || []).forEach(function (n) {
      ...
      if (n.group) el('div', 'swe-node-group', d).textContent = n.group;
      nodeEls[n.id] = d;
...
html = html.replace("__TITLE__", spec.get("title", "System Diagram"))
html = html.replace("__RENDERER_JS__", open(renderer).read())
html = html.replace("__DIAGRAM_JSON__", json.dumps(spec, indent=2))
...
### Reviewer — Round 1 (2026-07-04)

**Findings & proposals:**
- **[Blocker] Cyclic graphs blow up the layered layout instead of stabilizing it.**
  - *Reason:* `layout()` in `utils/swe-diagram/assets/renderer.js` uses repeated longest-path relaxation on the raw edge set. Any cycle (`A -> B`, `B -> A` is enough) keeps increasing ranks on each pass, so common bidirectional service relationships land in arbitrarily distant columns and the primary deliverable becomes misleading.
  - *Fix:* Condense strongly connected components first, then run longest-path layering on the DAG of SCCs; or explicitly treat back-edges/cycles as same-rank edges instead of relaxing them.
- **[Should] The HTML builder embeds raw JSON into a `<script>` tag and can break on valid repo text.**
  - *Reason:* `build-diagram.sh` injects `json.dumps(spec, indent=2)` directly into `renderDiagram(__DIAGRAM_JSON__);`. A valid description/title containing `</script>` closes the tag early and produces a broken HTML artifact. This is a robustness bug in the self-contained output path.
  - *Fix:* Emit the spec into `<script type=\"application/json\" id=\"diagram-spec\">...</script>` with HTML-safe escaping, then `JSON.parse(document.getElementById('diagram-spec').textContent)` before calling `renderDiagram`.
- **[Should] The declared `groups` schema is not actually honored by the renderer.**
  - *Reason:* The JSON contract defines `groups: [{ id, label }]`, but `renderer.js` never reads `spec.groups`; it sorts by `node.group` and renders the raw group id on each node. Following the example schema yields `backend` in the UI, not `Backend`, and makes the `groups` array dead weight.
  - *Fix:* Build a `groupId -> label` map from `spec.groups` and render the human label from that map; if grouped presentation is intentional, use the same map consistently in sorting/legending.
- **[Should] The change set misses the repo's skill authoring follow-through.**
  - *Reason:* The bundle includes `SKILL.md` plus assets, but no README update even though the review brief explicitly calls out "README updated in same change" as part of the repo convention check.
  - *Fix:* Add the matching README entry for `swe-diagram`, including when to trigger it and the two output artifacts it generates.
- **[Nit] The new skill files do not follow the repo's ASCII-punctuation convention.**
  - *Reason:* The skill text and renderer comments use non-ASCII punctuation/glyphs such as `—`, `→`, and `−`, while the review brief calls out ASCII punctuation as an authoring convention to honor.
  - *Fix:* Replace those glyphs with ASCII equivalents (`-`, `->`, `+/-`) unless there is a documented exception for user-facing output.
- **[Pass] The description does include concrete triggers and a counter-example.**
  - *Reason:* `SKILL.md` names multiple invocation phrases (`"draw the system diagram"`, `"diagram this repo/architecture"`, `"swe-diagram"`, etc.) and clearly excludes data charts/dashboards and single-algorithm flowcharts.
  - *Fix:* Keep this shape when tightening the other issues.

**Verdict:** Changes requested
VERDICT: FAIL
Basis: Static review of the embedded `swe-diagram` skill bundle against the relay brief, the stated repo conventions, and `GUIDING-PRINCIPLES.md`; no artifact/source edits or repo-wide gates run because this turn is review-only and relay-file-scoped.
