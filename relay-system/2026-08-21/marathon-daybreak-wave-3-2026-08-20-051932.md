...
Fixed the silent omission of session-derived issue numbers when `session.json` is malformed in `skills/standup/collect.sh`. 
- Modified `get_bounded_set` to catch `json.load()` exceptions and `isinstance(data, list)` shape errors, loudly degrading Lens 5 with `D5` instead of `pass`ing.
- Added test fixtures `lens-5-malformed-session` and `lens-5-invalid-shape-session`.
- Added corresponding assertions to `test/gh77-standup-triage.sh` to prove that Lens 5 degrades with `D5` when encountering malformed or invalidly shaped `session.json`.
The test suite passes (138 pass, 0 fail).
