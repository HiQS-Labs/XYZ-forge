...
repository tree and never writes anywhere but GitHub issues carrying its label:
  ...
  status != success  OR  any `wave-reconcile: SKIPPED …` line
      -> exactly one open issue labelled hosted-reconcile-attention: create it if none is open,
         otherwise comment on it. The body carries the run URL, the job status, the last
         `wave-reconcile: ERROR — …` line (or says none was captured) and every skip line.
...
def body_for(status, run_url, error, skips):
    lines = [f"Run: {run_url}", f"Job status: `{status}`", ""]
    lines.append(f"Terminal error: `{error[len(ERROR_PREFIX):]}`" if error
                 else "Terminal error: none captured (the run may have failed before or after the reconcile step).")
    if skips:
...
def main(argv=None):
    ...
    error, skips = summarize(read_log(args.log))
    attention = args.status != "success" or bool(skips)
    number = open_issue_number(args.label)

    if attention:
        body = body_for(args.status, args.run_url, error, skips)
        if number is None:
