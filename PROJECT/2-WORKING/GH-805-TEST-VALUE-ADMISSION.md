...
GitHub repo policy currently has no protection on development; Actions token defaults read-only and `can_approve_pull_request_reviews=false`. The organization is on Free, so an organization-required-workflow design is not available. Required status checks alone do not pin a workflow and are not the approval boundary. Use **native required CODEOWNER review by @noelsaw1, dismiss stale approvals, do not require a different last-push approver (the operator pushes task branches), enforce for administrators, no bypass actors**, plus a required deterministic admission check. This applies to all PRs; one coverage decision per code change (reuse/extend/add/no-add), not one approval per assertion. Operator approval is native GitHub review, never an agent-writable boolean or comment. Admin credentials remain administrative authority; this is not protection against a person/agent deliberately changing repository settings with that authority.
...
### Publication, credential and activation contract (plan QA clarification)

The native policy uses `require_last_push_approval=false`, `dismiss_stale_reviews=true`, `require_code_owner_reviews=true`, `required_approving_review_count=1`, and `enforce_admins=true`. This lets the sole operator approve bot-authored PRs even when the operator credential pushed the branch, while later commits still dismiss approval. The gateway authenticates the GitHub account, not a physical human: agent-held `noelsaw1` credentials can submit that account's reviews. Agents are forbidden to do so; **human-only enforcement requires removing review/admin capability from agent credentials**. Activation must surface and verify that credential separation; do not claim it from bot authorship. Prepare a concrete least-privilege credential setup (contents write, pull requests read, actions write; no repository administration) for operator completion rather than minting a reviewer identity or assuming different model names suffice.

Both publishers use job-scoped `GITHUB_TOKEN`, with Actions PR creation enabled at activation. After creating/updating the PR they explicitly dispatch `test-admission.yml` on **development** with the PR number, and `ci.yml` on the task branch with `publication_only=true` (blocking smoke, no advisory full canary). Deliberate full dispatch retains its false default and separate concurrency lane. Do not depend on token-generated pull_request_target events: they may be suppressed; pull_request workflow-run approval is separate from code review. Admission dispatch validates a positive PR number, loads the live PR via API, requires base development and an open PR, fetches its current head as Git objects only, validates with checked-out development code, and posts a uniquely named `test admission` check-run on that **head SHA** (pending/in-progress first, completed success/failure after). Compare the live head before completing; stale run cannot endorse a newer head. API/read/check publication failures fail the job, never fabricate green. Existing CI's workflow_dispatch supplies the smoke job; read back both exact-head checks before readiness. For normal author pushes, pull_request_target supplies the same path. No privileged candidate checkout or evaluation.

`development` is currently the default branch (live API read-back); deployment prerequisites require it still is, that both workflows and CODEOWNERS are landed there, and that bot dispatch is exercised before protection is enabled. Existing #811 is the bootstrap artifact; **no merge or activation is authorized by start-task**. Present final diff, policy JSON and verification for explicit operator handoff. Native branch review is authoritative even if a candidate attempts to spoof an Actions check; no workflow/status approval substitutes for the operator's review. No bypass actors, including Actions, are configured.

Protected reconciliation validates the original artifact allowlist before generating its packet, then allows that one tool-generated packet alongside those paths. Its no-add rationale describes lifecycle/receipt changes and zero new executable tests, and binds the complete artifact diff. An outstanding reconciliation PR pauses expensive new qualification. Catch-up excludes only bot-authored reconciliation PRs with a verified allowlisted diff, not arbitrary labels/titles/prefixes; otherwise a renamed code PR could evade qualification. If the protected mode cannot publish a PR, retain its branch/evidence and fail visibly, never fall through to a direct push.

### Ordered implementation and acceptance

1. [x] **Single decision command and disclosure — High, quick win.** Extend inventory CLI with prepare/check admission commands. Generate one `.github/test-admission/<id>.json` record per change set from a rationale file and a nonempty Git diff; bind the complete non-packet changed-file manifest (old/new blob IDs and modes) and merge-base, require outcome, behavior, existing coverage, reason, red evidence, cost/unknown explanation and issue. Report added/modified/deleted test files, registered-suite delta when parseable, and explicit unknown case counts; all executable changes require a decision, docs-only changes are explicitly classified. → An omitted file, added assertion inside an existing file, stale product edit, renamed/deleted test, malformed JSON and fabricated approval flag cannot pass; generated metadata is proposed, never approval.
...
4. [ ] **Authority-written suite budgets and content seal — High; content sealing is the quick win.** Extend the same admission writer/catalog with a per-suite approved ceiling, explicit counting unit, collected inventory digest and content fingerprints covering tests, relevant helpers and collection configuration. Approval must bind the exact proposed revision; the trusted authority writes the approved projection only after operator review, and agent-generated proposals cannot increase the trusted ceiling. → Adding/editing/deleting/renaming tests, changing parameterized cases, changing a helper, or hand-editing the budget refuses unless covered by a new authorized decision; unchanged counts do not excuse changed content. Existing suite registrations and framework-collected cases are separate units; unsupported shell case counts remain explicitly unknown and protected by fingerprints, never guessed from grep/assertion counts. Collector execution belongs in an unprivileged isolated runner, never the privileged base-code verifier. This new requirement needs plan/final QA before implementation and does not resolve the outstanding review-cap exception.
5. [ ] **Bounded proof and truthful handoff — High.** Extend existing gh419 and hosted-publisher tests; do not add a new suite. Run focused controls in a disposable full clone, retain provenance, obtain final Codex relay QA, then the final macOS gate and hosted check where executable before bootstrap. → Distinguish implemented and locally verified from activated on development; leave live policy/approval-path checkboxes open until actually witnessed. Retain #805's broader sample/value-analysis work as incomplete rather than equating gateway work with closing the whole issue.
...
### Gateway build progress

Plan relay round 2 approved b0ae4627. Decision preparation, complete manifest checking, early local refusal, trusted-base hosted publication, bot dispatch and protected reconciliation publication are implemented. Focused gh419/gh740/gh421/gh544 pass; three gateway mutations fail as expected with restored green. The native approval path and policy activation are not exercised yet: they require bootstrap landing and separated credentials, as documented in `docs/testing/TEST-ADMISSION.md`. Steps 2–4 remain open for final review/verification and live rollout; do not equate implementation with enforcement.
...
### Operator refinement — approved maximum and unauthorized-edit refusal

The operator requests an authority-written maximum for each admitted suite and a failing gate for edits/additions outside the writer. A maximum alone misses assertion replacement, delete/add swaps and parameter expansion; the acceptance contract is an authenticated approved inventory plus a content seal, with ceilings expressed in real runner units. The current proposal binding already detects post-generation changes but allows proposal regeneration; it is not an approved budget and must not be represented as one. There is no reliable way to prove which editor produced identical bytes, so the enforceable rule is that the exact content and budget must be authorized, regardless of editing method. Baseline legacy coverage can be frozen for growth control without claiming its value was audited or deleting tests to meet an arbitrary quota.
