...
### Phase 4 — Verify
- [x] **B3** carries `test/relay-uncited-findings.sh` (8 assertions: uncited `[Pass]` downgraded, unrelated
  findings untouched, inline-quote and file:line-cited findings left alone, uncited prose "verified"
  flagged, idempotent on a second pass, `RTL_WAS_REVIEWER_TURN` set correctly for both roles)
