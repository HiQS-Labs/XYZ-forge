...
2. A native scheduled wake is created only after the active harness confirms it can resume the current conversation. The agent reports the scheduler receipt and verifies the job exists. Red control: in a harness with no compatible scheduler, the skill must say no wake is armed and must not promise autonomous resumption. Evidence: plan/final QA review against official docs and available tools.
...
4. The scheduled job is cancelled in the same session when terminal; if cancellation fails, report the live job and manual stop action. Red control: deadline alone must not be claimed to delete a job automatically. Evidence: skill review.
