...
|---|---|
| Rev 2 of this plan after Codex plan-QA round 1 (Block: A.2/A.3 preservation, B1 resolver contract, B1/C acceptance, #526 dependency, parity guard). All ten findings accepted; dispositions in `relay-system/2026-09-09/gh534-merge-cleanup-plan-qa-codex.md` | Codex round 2 on this revision. **Implementation is blocked until PR #526 lands** (or the operator authorizes stacking on its pinned head) |
...
| D | `inspect_tick_claims()` defined, never called; `tick_claims` initialised false (`:181`), only the driver-lock helper runs (`:268`), so `ACTIVE_TICK_CLAIM` (`:296`) is unreachable. The helper itself is weak: reads one STATE heading and regular lock files, fails **open** on read errors (`:136-158`). SKILL.md `:51` additionally promises `lsof`, which exists nowhere | `scan_clones.py:136`, `:181`, `:268`, `:296`; `SKILL.md:51` | `grep -n inspect_tick_claims` → one hit, the definition |
...
2. **Squash exception, bound to provenance:** the tip SHA equals `headRefOid` of a PR whose
   ...
   (from `gh pr list --state merged --json number,headRefOid,mergeCommit,baseRefName`, base equal
   to the integration branch). Then the branch's **aggregate** diff from its merge-base to the tip
...
**A.4 Active-session evidence.** Call `inspect_tick_claims()` from `inspect_checkout()`, and fix
the helper so it **fails closed**: an unreadable `.tick/` or STATE file is a claim, not an absence.
Resolve the tick root through the git common dir for a linked worktree (the same resolver shape
as `driver-lock-lib.sh`), and treat a directory-shaped lock as a lock. `lsof` is either
implemented (open handles under the checkout → `ACTIVE_PROCESS`) or the promise is removed from
SKILL.md `:51`; recommend implementing, since the primary's Antigravity handle leak is the known
live case. Removing the promise does not make deleting an active clone safe.

**A.5 Fresh inspection before removal.** Phase 6 currently deletes from the inventory taken at
`merge_cleanup.py:262`. Re-run `inspect_checkout()` on each candidate immediately before
`teardown_checkout()`, after a fresh fetch; any new dirt, ref, stash, claim or process since the
scan blocks that removal. This is also the missing `PRESERVE_* → eligible` transition after a
successful Phase 5 merge: the second inspection sees the landed PR.
...
disposable clone of the PR head) is a subset of `{releases.db, releases.sql, ROADMAP-DASHBOARD.md,
LEADERBOARD.md, RELEASES-PREVIEW.html, LEADERBOARD.html}` is a B1 candidate. `harnesses.db` /
...
5. Regenerate views; run `releases check`; make the explicit merge commit; validate that head in a
   **second** disposable full clone (identity verified before and after); then push only if the
   remote PR head is still the SHA that was resolved. Pin source/head/base SHAs in the log.
...
- **Script emits** a handoff record per escalated PR: PR number, head/base/merge-base SHAs,
  unmerged file set with conflict types, hunk counts, which side last changed each file, the
  disposable clone path, and the attempt count. Written to a named location under `TESTS-RESULTS/`
  or `.tick/` so it survives resume.
- **Script enforces** a per-PR ceiling of two attempts across *all* rungs — bouncing between
  rungs or restarting does not reset it. A PR at its ceiling is parked with the record.
- **Script blocks dependents:** a PR that depends on a failed/parked predecessor is not attempted.
  ...
  alone cannot express this; Phase 5 tracks predecessor outcomes at runtime.
- **Caller (SKILL.md names each rung, in order):** `/debug-mantra` to establish the conflict as
  ground truth from the record; `/recon`, **read-only**, bounded to the files in the record, with
  an explicit file-count and time budget, stopping when exceeded; `/ponytail` for the smallest
  reviewed resolution — "independent hunks" is not proof of independent semantics, so a code
  resolution lands only with caller review and tests; `/start-task`-style handoff for semantic
  overlap; `/unstuck` when the record shows the same failure twice.
...
- Code rollback for everything else is `git revert`; the skill keeps no persistent state.

## Acceptance check

Each is a test in `test/gh436-merge-cleanup.py` unless stated; each red control is witnessed
during implementation with its output saved under `TESTS-RESULTS/<date>+GH-534/`.
...
**A.4** — `.tick/locks/x` → `ACTIVE_TICK_CLAIM`; unreadable `.tick/` → treated as claimed;
linked-worktree tick root resolves to the common dir; **red control: AST assertion that
`inspect_checkout` calls `inspect_tick_claims`**, failing when the call is deleted (a string or
comment must not satisfy it).

**A.5** — a candidate made dirty between scan and teardown is not removed; **red control: fails
when the fresh inspection is disabled**.
...
gate failure → no push; remote head moved → no push; `--dry-run` → zero mutation.

**C** — handoff record schema; per-PR ceiling survives a resume; a dependent of a parked PR is not
...
each row with owner `script` or `caller` and, for script rows, a test name. The guard parses the
table (nonempty), asserts every named test exists and runs, asserts described CLI options exist
in argparse, and asserts the D call at AST level. Negative controls in fixture copies, each
...
## Rating rationale — 2026-09-09 (rev 2)
