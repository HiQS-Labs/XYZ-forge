...
One tracked scratch file (`.relay-scratch/probe_telemetry.json`) made every real agent turn a
tracked-file mutation, tripping containment on work that touched nothing else. The fix (PR #185)
untracks the artifact and adds a derived guard (`test/gh184-no-tracked-scratch.sh`) asserting
nothing under `.relay-scratch/` is ever tracked — the guard matters more than the removal, because
the file was regenerated by routine gate runs and would have been re-committed by the next
innocent `git add`-broad sweep. Lesson: for runtime-generated paths, the durable fix is a
tracked-state invariant test plus gitignore coverage, not a one-time `git rm`; and any lane
directory an agent writes during normal operation must be born gitignored.
