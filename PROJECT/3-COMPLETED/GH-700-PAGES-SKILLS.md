...
| R3 | `sitemap.xml` lists `skills.html`, and every listed page's `lastmod` is bumped to `2026-09-18` (all 8 existing pages receive the new nav link, so their content changes) | `grep -c skills.html PAGES/sitemap.xml` = 1 **and** `grep -c '<lastmod>2026-09-18</lastmod>' PAGES/sitemap.xml` = 9 (red control: 0 at base; 8 `<loc>` rows dated 09-02/09-06) |
...
- `PAGES/sitemap.xml` — one new `<url>` row + `lastmod` → `2026-09-18` on all 9 rows
...
3. Update `sitemap.xml`: add the `skills.html` row and set `lastmod` to `2026-09-18` on all 9 rows (R3); repoint the Model Catalog card (R4).
