...
- **An array append resets `$?`.** `FAILED+=("$t")` is an assignment, and an assignment sets `$?` to 0. The first fix for the telemetry rc bug put `_rc=$?` *after* the append, so every failed sequential suite was recorded as `rc=0`. The rule is stricter than "capture rc early": capture it before **any** other statement, including a command substitution sitting in the same argument list (`rt_emit ... "$(rt_now_ms)" "$?"` has already lost it). Pinned by `gh365-validate-telemetry.sh` B5/B5b.
- **A green run cannot exercise a failure path.** That rc bug survived several full qualifying runs precisely because they all passed — no record ever carried a nonzero rc, so nothing could disagree. When a receipt's job is to describe failures, the only evidence it works is a run that *failed*. B5 forces `hq.sh` to fail rather than waiting for one.
- **An assertion's existence is not evidence it can fail.** B5b was verified by transposing the fix back out and confirming it reports `got '0'`; restored, 16/0. Do the mutation, every time. This branch shipped a vacuous assertion in review (an `awk` range that terminated on its own first line) and a second one while fixing the first (`not matches -- "$pat"`, where the helper already supplies `--`, so the extra one was consumed as the pattern and the check searched for the literal string `--`). Both passed against every negative control until the control failed to fire.
- **A failed command substitution produces an empty string, and an empty input makes a checker pass vacuously.** During review, two `git show "$B:ci-local.sh"` calls silently returned nothing — zsh consumed `:c` and `:t` as history modifiers — and the shell redirect created empty files anyway. A scanner then reported "CLEAN" against zero bytes. Size-check any extracted artifact before asserting anything about its contents.
- **Compare two live sources; never pin a derived literal.** The registry denominator moved 312 → 315 → 318 → 321 → 328 while this branch was open, and every review projection of it was stale on arrival. `D1` survived because it asserts `TESTS parse == validate.sh --list` rather than either number. Prefer `validate.sh --list` (the #141 read-only registry surface) to a `sed`/`grep` scrape of the `TESTS=(...)` block: same count today, but the scrape breaks on a formatting change and the interface does not.
- **Review the head you are about to write about, not the one you fetched.** The re-review at [5505077548](https://github.com/HiQS-Labs/XYZ-forge/pull/378#issuecomment-5505077548) restated a blocker that had already been fixed one push earlier, and asserted "unchanged" about code that had changed. Re-fetch immediately before writing a verdict on a branch someone else is actively pushing to.
- **A commit message that describes work its diff does not contain is invisible to a reviewer reading messages.** Two instances here (`858e5e0d`, `34aa7f16`), both disclosed by the author before a reviewer found them. Grep-verify the deliverable in the tree before writing the message that claims it, and read diffs rather than messages when auditing someone else's branch.
- **Concurrent appends from separate bash processes split JSON lines.** Pool workers are fresh `bash -c` processes reached through `xargs`; they inherit only exported functions, and a half-exported emitter surface once produced `"skip_lines":<empty>`. Each worker writes its own `$RT_FILE.w<BASHPID>` shard and the parent merges after the join — and un-shards *before* emitting the merge record, or that record lands in a fresh orphaned shard nothing folds in.
- **`grep -c` always prints its count and exits 1 on zero matches**, so `grep -c ... || printf '0'` prints `0\n0` and splits the record it was interpolated into. Capture first, default after.
- **BSD `tee` does not permute options**: `tee "$file" -a "$other"` treats `-a` as a filename and creates a file literally called `-a` in the working directory. The envelope's tracked-tree bracket is what caught it; without that bracket it would have dirtied the repo root on every suite silently.
- **A suite that stops asserting while exiting green is worse than a red one.** `gh346-gateway-allowlists.sh` reached the real driver via `python3 utils/py/marathon_drive.py` with no fixture root, retried the contended lock ~36s, then SKIPped its routing assertions and passed. The structural fix is a bidirectionally audited `DRIVER_LOCK_LANE` registry, not a longer hand-maintained string — a shipped-driver reference must be lane-serialized or carry a cited exemption.
- **Telemetry must degrade coarser, never corrupt.** `rt_now_ms` originally had no fallback; without perl it returned empty and every record became `"ts_ms":,` — unparseable, for the whole run. Best-effort means the run survives *and* the artifact stays readable: perl → `python3` → `date +%s`, losing sub-second ordering rather than the file.
