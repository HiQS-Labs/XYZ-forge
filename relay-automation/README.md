...
| `relay-turn-lib.sh` | **Shared safety core** (sourced, not run): the model-agnostic containment contract — path-allowlist + commit-bypass guard + no-push. Both headless turn-takers source this so the boundary lives in ONE place. See [decisions/2026-06-15-unattended-agent-containment.md](../decisions/2026-06-15-unattended-agent-containment.md). **Reviewer probes (GH-681):** `rtl_turn_prompt` tells a Reviewer it may run narrow, non-mutating probes against the seeded artifact, with output under `.relay-scratch/` (the harness's gitignored exception to AGENTS.md's `temp/` rule) or `$TMPDIR`; test suites stay out of the worktree. The containment sweep is unchanged; "read-only execution" is behavioural (relay Codex runs `-s workspace-write`), backstopped by the post-turn diff, and ignored-file residue outside `.relay-artifacts/` remains a known gap ([#682](https://github.com/HiQS-Labs/XYZ-forge/issues/682)). |
