...
# ($HERE/..). Explicit AGY_TURN_ROOT still wins (fixtures); fall back to $HERE/.. off a git repo.
ROOT="${AGY_TURN_ROOT:-"$(git rev-parse --show-toplevel 2>/dev/null || (cd "$HERE/.." && pwd))"}"
AGY_BIN="${AGY_BIN:-agy}"
...
if [[ -n "$wt" ]]; then
  ...
  if [[ "$bounded_rc" -eq 0 && -s "$AGY_LOG" ]]; then
    ...
    if grep -v -e '^\[trace\] ' -e 'TICK_REPO_ROOT=' -e 'file://' -e '](' "$AGY_LOG" 2>/dev/null | grep -F "$ROOT" >/dev/null 2>&1; then
